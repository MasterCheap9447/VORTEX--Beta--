...
func _physics_process(delta):
	...
	if !is_sliding && !is_dashing:
		if is_on_floor():
			...
			if direction:
				velocity.x = move_toward(velocity.x, direction.x * AIR_SPEED, AIR_ACCELERATION)
				velocity.z = move_toward(velocity.z, direction.z * AIR_SPEED, AIR_ACCELERATION)
